...
const MobileNav = ({ SignIn, SignUp }) => {
  ...
  const reduxState = useSelector((global) => global.UserReducer?.user);
    ...
    <>
      <div className="hidden lg:inline container px-20 mx-auto">
        <div className="hidden gap-4 w-full items-center justify-around lg:flex ">
          ...
          {reduxState?.UserReducer?.user?.user?.fullname ? (
            <div className="relative w-20">
              ...
              >
                <img
                  src={gravatar.url(reduxState?.UserReducer?.user.user.email)}
                  alt={reduxState?.UserReducer?.user.user.fullname}
                  className="w-full h-full rounded-full object-cover"
                />
              </div>
              {isDropDownOpen && (
